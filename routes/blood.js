...
router.route('/all/:bloodgroup&:userid').get((req, res) => {
    BloodPack.find({
        bloodgroup: paramsFinder(req.params.bloodgroup),
    })
        .then(bloodpacks => res.json(bloodpacks))
...
router.route('/:bloodgroup&:userid&:lat&:lng').get((req, res) => {
    BloodPack.find({
        bloodgroup: paramsFinder(req.params.bloodgroup),
        location: { $near: { $maxDistance: 100000, $geometry: { type: "Point", coordinates: [parseFloat(req.params.lng), parseFloat(req.params.lat)] } } }
    })
        .then(bloodpacks => res.json(bloodpacks))
        .catch(err => res.status(400).json('Error: ' + err));
});
