            ...
            <div className="requestfaqwrapper">
                {FAQs.map((info, index) => (
                    <div className='requestfaq' key={'requestfaq' + (index + 1).toString()}>
                        <button className="faqbox" onClick={this.toggleFAQs} value={(index + 1).toString()} style={{ borderTop: '1px solid #b21e2e', borderBottom: index === 5 ? '1px solid #b21e2e' : '' }}>{info}<FontAwesomeIcon style={{ pointerEvents: 'none', color: '#b21e2e' }} icon={this.state.active === (index + 1).toString() ? 'minus' : 'plus'} color='#ff8a82' /></button>
